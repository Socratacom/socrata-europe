...
define( 'CUSTOM_METABOXES_DIR', plugins_url() . '/socrata-case-studies/metaboxes' ); //Change this URL path. Also change the function prefix.
 ...
 */
function case_study_custom_meta_box_field( $field, $meta = null, $repeatable = null ) {
		...
		case 'repeatable':
			...
			foreach( $meta as $row ) {
				...
				foreach ( $repeatable_fields as $repeatable_field ) {
					...
					echo case_study_custom_meta_box_field( $repeatable_field, $meta[$i][$repeatable_field['id']], array( $id, $i ) );
					echo '</p>';
				} // end each field
 ...
 */
function case_study_meta_box_find_field_type( $needle, $haystack ) {
	foreach ( $haystack as $h )
		if ( isset( $h['type'] ) && $h['type'] == 'repeatable' )
			return case_study_meta_box_find_field_type( $needle, $h['repeatable_fields'] );
 ...
 * @return	bool				whether or not the type is in the provided array
 */
function case_study_meta_box_find_repeatable( $needle = 'repeatable', $haystack ) {
	foreach ( $haystack as $h )
 ...
 */
function case_study_meta_box_santitize_boolean( $string ) {
 ...
 */
function case_study_meta_box_sanitize( $string, $function = 'sanitize_text_field' ) {
	switch ( $function ) {
		case 'intval':
 ...
 */
function case_study_meta_box_array_map_r( $func, $meta, $sanitizer ) {
	...
	foreach( $meta as $key => $array ) {
		...
		foreach( $array as $arrayKey => $arrayValue )
			if ( is_array( $arrayValue ) )
				$array[$arrayKey] = case_study_meta_box_array_map_r( $func, $arrayValue, $newSanitizer[$arrayKey] );
 ...
 */
class case_study_custom_add_meta_box {
	...
	function admin_enqueue_scripts() {
		...
		if ( in_array( $pagenow, array( 'post-new.php', 'post.php' ) ) && in_array( get_post_type(), $this->page ) ) {
			// js
			$deps = array( 'jquery' );
			if ( case_study_meta_box_find_field_type( 'date', $this->fields ) )
				$deps[] = 'jquery-ui-datepicker';
			if ( case_study_meta_box_find_field_type( 'slider', $this->fields ) )
				$deps[] = 'jquery-ui-slider';
			if ( case_study_meta_box_find_field_type( 'color', $this->fields ) )
				$deps[] = 'farbtastic';
			if ( in_array( true, array(
				case_study_meta_box_find_field_type( 'chosen', $this->fields ),
				case_study_meta_box_find_field_type( 'post_chosen', $this->fields )
			) ) ) {
			...
			if ( in_array( true, array( 
				case_study_meta_box_find_field_type( 'date', $this->fields ), 
				case_study_meta_box_find_field_type( 'slider', $this->fields ),
				case_study_meta_box_find_field_type( 'color', $this->fields ),
				case_study_meta_box_find_field_type( 'chosen', $this->fields ),
				case_study_meta_box_find_field_type( 'post_chosen', $this->fields ),
				case_study_meta_box_find_repeatable( 'repeatable', $this->fields ),
				case_study_meta_box_find_field_type( 'image', $this->fields ),
				case_study_meta_box_find_field_type( 'file', $this->fields )
			) ) )
			...
			if ( case_study_meta_box_find_field_type( 'date', $this->fields ) || case_study_meta_box_find_field_type( 'slider', $this->fields ) )
				$deps[] = 'jqueryui';
			if ( case_study_meta_box_find_field_type( 'color', $this->fields ) )
				$deps[] = 'farbtastic';
	 ...
	 */
	function admin_head() {
		if ( in_array( get_post_type(), $this->page ) && ( case_study_meta_box_find_field_type( 'date', $this->fields ) || case_study_meta_box_find_field_type( 'slider', $this->fields ) ) ) {
		...
		foreach ( $this->fields as $field) {
			if ( $field['type'] == 'section' ) {
				...
				echo '<tr>
						...
						echo case_study_custom_meta_box_field( $field, $meta );
		...
		foreach ( $this->fields as $field ) {
			...
			if( in_array( $field['type'], array( 'tax_select', 'tax_checkboxes' ) ) ) {
				...
				if ( isset( $new ) && '' == $new && $old ) {
					...
					if ( is_array( $new ) )
						$new = case_study_meta_box_array_map_r( 'case_study_meta_box_sanitize', $new, $sanitizer );
					else
						$new = case_study_meta_box_sanitize( $new, $sanitizer );
